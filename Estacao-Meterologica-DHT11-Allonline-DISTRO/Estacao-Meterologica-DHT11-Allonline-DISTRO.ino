...
void setup_wifi();
void setup_broker();
void pub_allOneLine(NTPClient time,float temperatura, float umidade);
...
DHT dht(D3, DHT22); // sensor DHT11 ou DHT22?  montado no pino D3 ou Dx?
long lastMsg = 0;
...
void loop()
{
          ...
          if (now - lastMsg > INTERVALO) {
                ...
                pub_allOneLine(timeClient, temperatura, umidade);
...
void pub_allOneLine(NTPClient time,float temperatura, float umidade)
{
  ...
  String topic ;
  String msg = "";
  topic = rtopico + (String)"/all";
  for (int i = 0; i < 6; i++)
    msg = msg + (String)tnow[i] + sep[i];
  msg = msg + (String)temperatura + sep[5]+ (String)umidade ;
